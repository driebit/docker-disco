...
# We can't know the host IP so we need festina to tell us.
echo '{"advertise_addr": "'$(curl -m 2 http://ip.dev/)'", "node_name": "'${DOCKER_USER}'"}' > /consul/config/advertise_addr.json

# Add a service definition
echo '{"service": { "name": "'${DOCKER_SERVICE}'", "tags": ["'${DOCKER_USER}'"], "port": '${DOCKER_PORT}'} }' > /consul/config/service.json
